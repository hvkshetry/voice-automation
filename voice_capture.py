...
class VoiceCapture:
        ...
        try:
            ...
            if response.status_code == 200:
                        ...
                        if line.startswith('data: '):
                            try:
                                ...
                                if data.get("type") == "message":
                                    ...
                                    # Speak response immediately with voice from bridge
                                    if self.config["tts"]["enabled"] and response_text:
                                        self.speak_with_voice(response_text, voice, voice_config)
                                        break  # Exit after first message for now
                ...
                if response_text:
                    ...
                    logger.warning("No response text received from streaming")
            else:
    ...
    def speak_with_voice(self, text: str, voice: str = None, voice_config: dict = None):
        """Send text to TTS service with specific voice configuration"""
        if not text:
            return
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Log the response but don't fail silently
                logger.info(f"Text was: {text[:100]}...")
        
        except Exception as e:
            logger.error(f"TTS error: {str(e)}")
            # Log the text that couldn't be spoken
            logger.info(f"Text was: {text[:100]}...")
        finally:
    ...
    def speak(self, text: str):
        """Send text to TTS service with default voice"""
        self.speak_with_voice(text)
